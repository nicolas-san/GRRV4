...
$grr_script_name = 'day_export_xml.php';
$tplArray = [];
// Settings
...
// Resume session
/* désactiver pour permettre l'accès direct aux infos il faudrait le protéger par ip ce fichier */
...
if (!$res) {
    ...
    for ($i = 0; ($row = grr_sql_row($res, $i)); ++$i) {
        ...
        while ($t <= $end_t) {
            ...
            $type_name = grr_sql_query1('SELECT type_name from '.TABLE_PREFIX."_type_area where type_letter='".$row['6']."'");
            if ($type_name == -1) {
                $type_name = "?$type?";
            }
            $d[$day_num]['typeName'][] = $type_name;
           /* var_dump($row[6]);
            var_dump($type_name);*/
            $d[$day_num]['res'][] = $row['7'];
    ...
    */
    ...
    for ($ir = 0; ($row = grr_sql_row($res, $ir)); $ir++) {
        ...
        if ($verif_acces_ressource) {
            ...
            $tplArray['rooms'][$incrementRoomAccessible]['name'] = $row[0];
                                ...
                                /*if ($d[$cday]['id_room'][$i]['tStart'] == $tCreneaux) {*/
                                    ...
                                    if ($d[$cday]['id_room'][$i] == $row['2']) {
                                        ...
                                        if ($acces_fiche_reservation) {
                                            ...
                                            $tplArray['rooms'][$incrementRoomAccessible]['jours'][$k]['reservations'][$i]['typeName'] = $d[$cday]['typeName'][$i];
...
if ($_GET['pview'] == 1) {
    ...
    echo $twig->render('day_export_xml.html.twig', $tplArray);
}
?>
