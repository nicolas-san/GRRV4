...
if ((isset($_GET['action'])) && ($_GET['action'] == 'synchro') && (Settings::get('sso_statut') == 'lcs')) {
    ...
    for ($loop = 0; $loop < $total_user; $loop++) {
        ...
        /*$user_tel = $user['tel'];
        $user_champs_libre = $user['champs_libre'];*/
...
?>
<td>
	<?php echo get_vocab('display_user_on.php'); ?><input type="radio" name="display" value='actifs' <?php if ($display == 'actifs') {
    echo ' checked="checked"';
} ?> /></td>
	<td>
		<?php echo get_vocab('display_user_off.php'); ?><input type="radio" name="display" value='inactifs' <?php if ($display == 'inactifs') {
    echo ' checked="checked"';
} ?> /></td>
...
echo "<td><b><a href='admin_user.php?order_by=nom,prenom&amp;display=$display'>".get_vocab('tel').'</a></b></td>';
...
echo '</tr>';
$sql = 'SELECT nom, prenom, statut, login, etat, source, tel, champs_libre FROM '.TABLE_PREFIX."_utilisateurs ORDER BY $order_by";
$res = grr_sql_query($sql);
if ($res) {
    for ($i = 0; ($row = grr_sql_row($res, $i)); ++$i) {
        ...
        if (($user_etat[$i] == 'actif') && (($display == 'tous') || ($display == 'actifs'))) {
            $affiche = 'yes';
        } elseif (($user_etat[$i] != 'actif') && (($display == 'tous') || ($display == 'inactifs'))) {
            $affiche = 'yes';
        } else {
            $affiche = 'no';
        }
