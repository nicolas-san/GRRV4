    ...
    if (($reg_nom == '') || ($reg_prenom == '')) {
        ...
        if ((isset($new_login)) && ($new_login != '') && ($test_login == '')) {
            ...
            if (!($test_statut)) {
                ...
                $retry = 'yes';
            } elseif ((($reg_password != $reg_password2) || (strlen($reg_password) < $pass_leng)) && ($reg_type_authentification == 'locale')) {
                $msg = get_vocab('passwd_error');
                $retry = 'yes';
            } else {
                ...
                if ($nombreligne != 0) {
					...
					champs_libre='".protect_data_sql($reg_champs_libre)."',
					default_site = '-1',
					default_area = '-1',
					default_room = '-1',
					default_style = '',
...
//
        } elseif ((isset($user_login)) && ($user_login != '')) {
            // un gestionnaire d'utilisateurs ne peut pas modifier un administrateur général ou un gestionnaire d'utilisateurs
            $test_statut = true;
            if (authGetUserLevel(getUserName(), -1) < 6) {
            ...
            if (!($test_statut)) {
                $msg = get_vocab('erreur_choix_statut');
                $retry = 'yes';
            } elseif ($reg_type_authentification == 'locale') {
    ...
    if ($retry == 'yes') {
        ...
        $user_etat = $reg_etat;
        $user_tel = $reg_tel;
        $user_champs_libre = $reg_champs_libre;
    }
}
// On appelle les informations de l'utilisateur pour les afficher :
if (isset($user_login) && ($user_login != '')) {
    $sql = 'SELECT nom, prenom, statut, etat, email, source, tel, champs_libre FROM '.TABLE_PREFIX."_utilisateurs WHERE login='$user_login'";
    $res = grr_sql_query($sql);
    if ($res) {
...
// un gestionnaire d'utilisateurs ne peut pas créer un administrateur général ou un gestionnaire d'utilisateurs
    ...
    echo "</tr>\n";
    echo '</table>';
    echo "<div class=\"form-group col-md-3\"><label for=\"reg_tel\">".get_vocab('tel').get_vocab('deux_points')."</label><input type=\"text\" name=\"reg_tel\" value=\"$user_tel\" class=\"col-md-3 form-control\" /></div><div class=\"clearfix\"></div>";
    echo "<div class=\"form-group\"><label  for=\"reg_champs_libre\">".get_vocab('champs_libre').get_vocab('deux_points')."</label><input type=\"text\" name=\"reg_champs_libre\" value=\"$user_champs_libre\" class=\"col-md-10 form-control\" /></div>";
